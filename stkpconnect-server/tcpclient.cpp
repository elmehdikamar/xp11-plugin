  ...
  , _refProvider(refProvider)
{
    ...
    out << "STKPCONNECT " << STKPCONNECT_PROTOCOL << "\n";
    out << "STKPCONNECT-VERSION " << STKPCONNECT_VERSION << "\n";
    out.flush();
    _socket->write(block);
}
...
void TcpClient::stkpconnectWarning(QString message) {
    if(_socket->isOpen()) {
        _socket->write(QString("STKPCONNECT-WARNING %1\n").arg(message).toUtf8());
    }
}
...
void TcpClient::readClient() {
    while(_socket->canReadLine()) {
        ...
        if(command == "disconnect") {
            ...
            if(subLine.length() >= 2) {
                ...
                if(subLine.length() >= 3) {
                    ...
                    if(!accuracy_ok) {
                        stkpconnectWarning(QString("WARNING: Unable to parse accuracy value: %1 - accuracy set to 0").arg(subLine[2]));
                    }
                }
                ...
                if(ref && ref->isValid()) {
                    sendRef(ref); // THING THAT BREAKS
                    if(command == "get") ref->setUnsubscribeAfterChange();
                }
            } else {
                stkpconnectWarning(QString("Invalid sub command"));
            }
            ...
            if(subLine.size() == 3) {
                ...
                if (ref) {
                    if(ref->isWritable()) {
                        ...
                        stkpconnectWarning(QString("Ref %1 is not writable!").arg(ref->name()));
                    }
                }
            } else {
                stkpconnectWarning(QString("Invalid set command"));
            }
        } else if(command == "key") {
            if(subLine.size() == 2) {
        ...
        } else if(command == "stkpconnect-set") {
            if(subLine.size()==3) {
                if(subLine.value(1) == "update_interval") {
                    ...
                    if(ok) {
                        ...
                        stkpconnectWarning(QString("Invalid flight loop interval %1").arg(subLine.value(2)));
                    }
                } else {
                    stkpconnectWarning(QString("Invalid update_interval command"));
                }
            } else {
                stkpconnectWarning(QString("Invalid stkpconnect-set command"));
            }
        } else if (command == "cmd") {
            if (subLine.size()==3) {
                QString subCmd = subLine.value(1);
                stkpconnectCommandType type = stkpconnectCommandTypeInvalid;
                if (subCmd == "once") {
                    type = stkpconnectCommandTypeOnce;
                } else if (subCmd == "begin") {
                    type = stkpconnectCommandTypeBegin;
                } else if (subCmd == "end") {
                    type = stkpconnectCommandTypeEnd;
                } else {
                    stkpconnectWarning(QString("Invalid cmd command"));
                }
            ...
            _refProvider->move(subLine.value(1));
        } else if(command == "sit"){
            if(subLine.size() == 2) {
                _refProvider->loadSituation(subLine.value(1));
            } else {
                stkpconnectWarning(QString("Invalid sit command"));
            }
            ...
            stkpconnectWarning(QString("Unknown command %1").arg(command));
        }
    }
}
...
void TcpClient::refChanged(DataRef *ref) {
    ...
    if(ref->type()== stkpconnectRefTypeFloat) {
        ...
        stkpconnectWarning(QString("Ref type %1 not supported (this should not happen!)").arg(ref->type()));
        return;
    }
